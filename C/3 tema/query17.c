...
int has_dublicate_different_grade(char string_array[][30], int length, int grades[])
{
    for(int i=0; i < length - 1; i++)
        for(int j=i+1; j < length; j++)
            if(strcmp(string_array[i], string_array[j]) == 0 && grades[i] != grades[j])
                return 1;
...
int main(int argc, char *argv[]) {
	...
	if (db){
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
            ...
            if(has_dublicate_different_grade(s.courses, s.load, s.grades))
            {
